...
def make_anchors(feats, strides, grid_cell_offset=0.5):
    ...
    for i, stride in enumerate(strides):
        ...
        sy, sx      = torch.meshgrid(sy, sx, indexing='ij') if TORCH_1_10 else torch.meshgrid(sy, sx)  # hw
        anchor_points.append(torch.stack((sx, sy), -1).view(-1, 2))  # hw,2
        stride_tensor.append(torch.full((h * w, 1), stride, dtype=dtype, device=device))  # hw,1
...
def onnx_decode_box(dbox, cls, origin_cls, anchors, strides):
    input_shape = [640,640]
    ...
    y[:, :, :4] = y[:, :, :4] / torch.Tensor(
        [input_shape[1], input_shape[0], input_shape[1], input_shape[0]]).to(y.device)
    return y
...
class DecodeBox():
    ...
    def non_max_suppression(self, prediction, num_classes, input_shape, image_shape, letterbox_image, conf_thres=0.5, nms_thres=0.4):
        ...
        #   prediction  [batch_size, num_anchors, 85] 84
        #----------------------------------------------------------#
...
if __name__ == "__main__":
    ...
    def get_anchors_and_decode(input, input_shape, anchors, anchors_mask, num_classes):
        ...
        img = Image.open("../2288/ZDSelevatorgoods20230531_V3_train_elevator_1_002762.jpg").resize([640, 640])
